...
function handleMapClick(e: L.LeafletMouseEvent) {
  ...
  if (appStore.markerMode) {
    ...
    const target = snapped ? { lat: snapped.lat, lng: snapped.lng, name: snapped.name } : { lat, lng, name: null as string | null }
    addAnnotationMarker(target.lat, target.lng, target.name)
...
function snapToNearestFeature(lat: number, lng: number, pixelTolerance: number): { lat: number; lng: number; name: string } | null {
  ...
  // bestHit 在闭包内赋值，TS 控制流无法跨闭包追踪；用 any 避免错误推断
  ...
  layerMap.forEach((leafletLayer, layerId) => {
    ...
    children.forEach((child, idx) => {
      ...
      if (dist <= pixelTolerance && (!bestHit || dist < bestHit.dist)) {
        const props: any = (Array.isArray(data.properties) && data.properties[idx]) ||
          (Array.isArray(data.features) && (data.features as any)[idx]?.properties) || {}
        const c: any = center
        bestHit = { lat: c.lat, lng: c.lng, name: props.name || props.NAME || data.name || '', dist }
      }
    })
  })
  return bestHit ? { lat: bestHit.lat, lng: bestHit.lng, name: bestHit.name } : null
}
