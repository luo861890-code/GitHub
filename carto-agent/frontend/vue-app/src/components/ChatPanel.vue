    ...
    <div ref="messageListRef" class="chat-messages">
      ...
      >
        ...
        <div class="message-body">
          ...
          <!-- 智能制图过程（Agent Trace，研究基线版 §33） -->
          <div
            v-if="((msg.map_data as any)?.provenance || msg.steps?.length)"
            class="message-trace-link"
            @click.stop="openTrace(msg)"
          >
          ...
          <div
            v-if="(msg.map_data || msg.map_id)"
            class="message-trace-link qa-link"
            @click.stop="openQaReport(msg)"
          >
...
/** 打开智能制图过程 Trace 面板（研究基线版 §33：Agent 可解释性） */
async function openTrace(msg: Message) {
  // 消息仅存轻量地图引用（map_id），完整 provenance 需按需从地图文件拉取
  let provenance = (msg.map_data as any)?.provenance || {}
  if ((!provenance || Object.keys(provenance).length === 0) && msg.map_id) {
    try {
      const res = await api.getMap(msg.map_id)
      const data = res.data || res
      if (data?.provenance) provenance = data.provenance
    } catch (e) {
      console.error('加载地图 provenance 失败', e)
    }
  }
  appStore.openTracePanel({
    provenance,
    steps: msg.steps || [],
  })
}
...
/** 打开地图质量验收报告（1000 分制） */
async function openQaReport(msg: Message) {
  const mapId = (msg.map_data as any)?.map_id || msg.map_id
  if (!mapId) return
  try {
    const res = await api.getMapQa(mapId)
    const data = res.data || res
    if (data?.total_score != null) {
      appStore.openQaPanel(data)
    } else {
      alert('质量验收失败：' + (data?.message || '无报告'))
    }
  } catch (e: any) {
    alert('质量验收失败: ' + e.message)
  }
}
